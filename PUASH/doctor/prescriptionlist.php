...
//     $author=$row1['fullName'];
//     $_SESSION['author']=$author;



// }
$query1=mysqli_query($con,"select * from doctors where id='".$_SESSION['id']."'");
					while($row11=mysqli_fetch_array($query1))
					{
						
						$author=$row11['doctorname'];
						$_SESSION['author']=$author;
...
{
$history= "<script>document.write(col1);</script>";
if($history==null){
     echo "<script>alert('##');</script>";
}
else{
    echo "<script>alert($history);</script>";
}
		  }
...
<html lang="en">
	<head>
		<title>Doctor | Prescription List </title>
		<meta charset="utf-8" />
			...
			<div class="app-content">
				...
				<div class="main-content" >
					<div class="wrap-content container" id="container">
						...
						<form type='POST' action='prescriptioncontroler.php'>
...
$sql=mysqli_query($con,"select id, name,pid,phone,gender from prescription where doctorsauthor='".$_SESSION['id']."'");
$cnt=1;
...
?>
                        ...
                        ?></td>
						<td name="ID"  style='display:none'><?php
                        echo $row['id'];
...
<td>
                        ...
                        ?>
							...
							<Button type ="submit" name='submit' id="submit_p" class="submit btn btn-xs btn-primary" style='float:left;margin-right:5px;'>
							 <i class="fa fa-eye">Prescription
						...
						<form action="history.php" style='float:left;'>
	   <a href="history.php" id='history_b' style='text-decoration: none;
    background: #007aff;
    color: #fff;
    padding: 4px 10px;
    border-radius: 3px;
...
$("#table").on('click','#history_b',function(){
         ...
         var col2=currentRow1.find("td:eq(2)").text(); // get current row 1st TD value
