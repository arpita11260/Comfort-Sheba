...
class Post {
	...
	public function submitPost($title,$body,$imageName) {
		...
		if($check_empty != "") {
			...
			//insert post 
			$query = mysqli_query($this->con, "INSERT INTO posts(author,title,description,upload_date,image,deleted) VALUES('$added_by', '$title','$body', '$date_added', '$imageName','no')");
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				// }
					...
					$user_to = "to <a href='" . $row['author'] ."'>" . $user_to_name . "</a>";
